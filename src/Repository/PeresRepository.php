...
class PeresRepository extends ServiceEntityRepository
{
    ...
    public function add(Peres $entity, bool $flush = false): void
    {
    ...
    public function findByAll(): array
    {
        return $this->createQueryBuilder('p')
            ->addSelect('pr', 't1', 't2','n','pren') // Charge toutes les relations
            ->leftJoin('p.profession', 'pr') // <-- Remplacer innerJoin par leftJoin
            ->leftJoin('p.telephone1', 't1') // <-- Remplacer innerJoin par leftJoin
            ->leftJoin('p.telephone2', 't2') // <-- Remplacer innerJoin par leftJoin
            ->leftJoin('p.nom', 'n') // <-- Remplacer innerJoin par leftJoin
            ->leftJoin('p.prenom', 'pren') // <-- Remplacer innerJoin par leftJoin
    ...
    /**
     * Summary of findBySearchData
     * @param \App\Data\SearchData $searchData
     */
    public function findBySearchData(SearchData $searchData)
    {
     ...
     /**
      * Summary of findBySearchParentData
      * @param \App\Data\SearchParentData $searchParentData
      */
